...
This study provides the first behavioral evidence on artificial intelligence (AI) use in tourism occupations, drawing on the Anthropic Economic Index (AEI), an open dataset that maps millions of privacy-protected Claude conversations to Standard Occupational Classification (SOC) categories. Unlike survey-based adoption measures, the AEI records what workers actually ask AI systems to do. Using the June 2026 release covering 121 countries, I construct a tourism-SOC intensity measure from the two major groups most typical of tourism employment (Food Preparation and Serving Related; Personal Care and Service) and relate it to countries' overall AI usage intensity and to economic and tourism indicators from the World Bank. Three findings emerge. First, tourism-occupation AI usage is small in absolute terms—fewer than 2.5% of observed work conversations in almost all countries—but varies substantially across countries (0.7% to 2.9%), and is systematically *higher* in less developed economies: it correlates negatively with GDP per capita (r = −0.30, 95% CI [−0.41, −0.19]) and with overall Claude usage intensity (r = −0.36), and positively with the share of travel services in exports (r = +0.17). Second, this pattern inverts the usual "digital divide" ordering and is consistent with a composition effect: tourism employment looms larger in the occupational structures of developing economies, while knowledge-intensive occupations absorb most AI usage in rich ones. Third, at the task level, tourism conversations cluster toward automation (repetitive drafting, scheduling, and templated communication) rather than augmentation, and hospitality occupations show among the highest "human-only ability" shares in the AEI, indicating that observed AI use in tourism work concentrates on the sector's administrative and creative periphery rather than its core service encounters. The paper contributes a reproducible, transparent framework for monitoring AI adoption in tourism labor markets with behavioral data, and discusses implications for tourism economics research, workforce policy, and the interpretation of AI-driven productivity claims in service industries.
...
The contribution is fourfold. First, it is—to the author's knowledge after a systematic search of the literature (Section 2.3)—the first study to combine behavioral AI-usage data with tourism economics. Prior work using the AEI has examined aggregate macroeconomic adoption patterns (Appel et al., 2025; Yildirim, 2026) and regional U.S. patterns (Noll et al., 2025), but not tourism occupations or tourism development. Second, it introduces a transparent, fully reproducible measure—tourism SOC intensity—that derives from published, auditable data rather than a constructed survey index. Third, it documents an empirical pattern that contradicts the intuitive "rich countries adopt AI faster" prior: tourism-occupation AI usage is *higher* in poorer economies, a composition effect with policy implications for tourism-dependent developing countries. Fourth, it demonstrates a monitoring framework that tourism scholars and statistical agencies can replicate each AEI release without proprietary data or fieldwork.

Previewing the results: tourism-occupation AI usage averages about 1.05% of country-level work conversations (the mean of the Food Preparation and Serving Related and Personal Care and Service group shares) and ranges from 0.66% (Tunisia) to 2.87% (Kyrgyz Republic). Richer countries show *lower* tourism intensity (β on log GDP per capita ≈ −0.11 to −0.18), driven by composition: in rich economies, AI conversations concentrate overwhelmingly in computer, mathematical, and management occupations, so tourism groups shrink as a share even as their absolute usage grows. The one tourism-specific indicator that correlates positively with tourism-occupation AI usage is the share of travel services in service exports (r = +0.17, CI [0.04, 0.33])—economies more dependent on tourism exports show more tourism-occupation AI activity relative to their overall usage. At the task level, tourism occupations skew toward automation rather than augmentation, and several hospitality occupations (Waiters and Waitresses, Concierges, Flight Attendants) show human-only ability shares above 97%, meaning that the AI conversations attributed to them overwhelmingly involve tasks adjacent to—rather than inside—core service delivery.
...
Figure 1 shows the global distribution of Claude work-related conversations across SOC major groups. Computer and Mathematical dominates at about 23% of conversations. The tourism-relevant groups are dispersed: Arts, Design, Entertainment, Sports, and Media is the second-largest group overall (about 13.5%), while Sales and Related is about 8.7%; Personal Care and Service, Food Preparation and Serving Related, and Transportation and Material Moving each account for about 1% or less. The immediate implication is that tourism-typical occupations are a small slice of behavioral AI usage at the global level—far below what "tourism is highly exposed to AI" narratives might suggest—and that any country-level analysis must treat these shares as composition-sensitive.

Table 1 profiles 19 tourism-relevant detailed occupations globally. Two regularities stand out. First, the largest tourism-occupation shares belong to managerial and coordination roles—Food Service Managers (0.13%), Meeting, Convention, and Event Planners (0.12%), Recreation Workers (0.11%)—rather than front-line service roles. Second, the classic front-line occupations are nearly invisible: Waiters and Waitresses (0.17% of global conversations but the largest single detailed tourism occupation), Tour Guides and Escorts (0.03%), Travel Agents (0.01%), Flight Attendants (0.01%), Hotel, Motel, and Resort Desk Clerks (0.00%). Table 1 also shows task-level metrics for the twelve largest: use-case shares indicate that for most of these occupations, only a minority of conversations are work-related (e.g., Waiters and Waitresses: 2.8% work), meaning much observed "tourism-occupation" AI use is personal use by people in those occupations—a further composition caveat that the analysis treats explicitly.

### 4.2 Cross-country patterns

Figure 2 plots tourism SOC intensity against log GDP per capita. The relationship is negative and moderately strong: the bivariate correlation is −0.30 (CI [−0.41, −0.19]). Table 2 reports the full correlation set. Tourism intensity correlates negatively with overall Claude usage intensity (r = −0.36), services employment share (r = −0.34), services value added (r = −0.38), internet use (r = −0.30), and mobile subscriptions (r = −0.29); and positively with travel services share of service exports (r = +0.17, CI [0.04, 0.33]). The tourism-specific indicators—arrivals per capita and receipts per capita—are negative but not significant at conventional levels.
...
The interpretation has a policy edge. For tourism-dependent developing economies, the observed tourism AI share reflects their occupational structure, not accelerated AI adoption in tourism firms. Conversely, the near-invisibility of front-line tourism occupations in high-income countries' AI conversations (Section 4.1) suggests that behavioral AI use has so far concentrated in the sector's administrative and creative periphery—exactly the tasks that exposure indices flagged as most susceptible (Eloundou et al., 2023)—while core service encounters remain, behaviorally, human territory.
...
At the detailed level (Table 1, right columns), hospitality occupations show extreme human-only ability shares: Concierges (100%), Travel Agents (99.1%), Waiters and Waitresses (99.5%), Flight Attendants (97.7%). Mean AI autonomy scores cluster between 2.3 and 3.0 on the 1–5 scale, below the global detailed-occupation average of roughly 3.3. Together, these metrics say that the AI conversations attributed to tourism occupations rarely involve tasks the AI can fully own: they are drafting, planning, and information-adjacent tasks performed *alongside* human service roles, not substitutions for them.

### 4.5 Robustness

Three checks (Appendix B, Tables B1–B3) probe the main pattern. First, alternative tourism intensity definitions—sum instead of mean; adding Sales and Related; adding Arts, Design, Entertainment, Sports, and Media—preserve the negative GDP per capita association (β between −0.08 and −0.19) and the positive travel-exports association. Second, excluding the five countries with the highest tourism intensity leaves the income gradient intact (β = −0.10, t = −3.6), so the pattern is not driven by small tourism-economies at the top of the range. Third, using April 2026 instead of May 2026 data reproduces all correlations within ±0.02, indicating month-to-month stability of the composition pattern.
...
Three limitations bound the interpretation. First, *platform composition*: the AEI observes Claude users, who skew toward high-income, English-proficient, digitally engaged populations. The cross-country income gradient in tourism intensity therefore partially reflects who in each country uses Claude, not who uses AI. Second, *occupation attribution*: conversations are classified by inferred occupation, and classification error is not publicly quantified; some conversations attributed to tourism occupations may belong to users exploring tourism topics personally (though the use-case metrics partially address this). Third, *industry-agnostic classification*: SOC codes describe occupations, not industries, so "tourism occupations" capture where tourism *work* is done, not all work done *in* tourism industries—managers in tourism firms appear under Management, and drivers under Transportation. The measure is best read as tourism-*work* intensity, a boundary the paper maintains throughout. A fourth caveat is *cross-sectional design*: with two monthly observations, the analysis identifies associations, not dynamics or causal effects.

### 5.3 Implications

For *research*, the results caution against extrapolating from exposure indices to adoption, and suggest that tourism AI research distinguish carefully between the intensive and compositional margins. For *policy*, the positive association between travel-export dependence and tourism-occupation AI usage, combined with the near-absence of front-line usage, implies that tourism AI strategies in developing economies currently touch administrative and creative support tasks; programs targeting core service work would be operating where behavior shows little AI activity yet. For *statistics*, the replication framework offers national tourism administrations a low-cost instrument for monitoring AI adoption between official surveys.

## 6. Conclusion

This paper introduced behavioral evidence on AI use in tourism occupations using the Anthropic Economic Index, the first tourism application of this data source. Tourism-typical occupations account for a small but measurable share of AI conversations across 121 countries; their share is higher in poorer economies and in economies more dependent on travel exports, and the AI conversations that do occur in tourism occupations lean toward automation and away from core service delivery. The framework is transparent, reproducible, and repeatable with each AEI release, offering tourism economics a living instrument for tracking how the sector's work is—and is not—being reshaped by AI.
...
Noll, B., Cason, T., Cheche, O. K., Brown, W., & Peet, E. (2025). *Artificial intelligence and the Anthropic Economic Index in the Mountain West*. University of Nevada, Las Vegas.
...
| Museum Technicians and Conservators | 0.00 | — | — | — | — | — |
| Travel Guides | 0.00 | — | — | — | — | — |
| Amusement and Recreation Attendants | 0.00 | — | — | — | — | — |

*Notes:* Global-level AEI shares, May 2026. Dashes indicate metrics not published for that occupation (below sample floors). "Automation/Augmentation" are collaboration-bucket shares; "Human-only ability" is the share of conversations on tasks a human could complete without AI. Full data: `results/table1_tourism_occupations.csv` and `results/table5_detail_profiles.csv`.
...
| International arrivals per capita (log) | −0.14 | [−0.28, 0.00] | 107 |
| Tourism receipts per capita (log) | −0.12 | [−0.27, −0.03] | 99 |
...
On August 27, 2026, the author searched OpenAlex (`api.openalex.org`), Google Scholar, SSRN, and arXiv with term combinations: ("Anthropic Economic Index" OR "Claude conversations" OR "Claude usage") AND ("tourism" OR "hospitality" OR "travel" OR "accommodation"); ("tourism" OR "hospitality") AND ("occupational exposure" OR "AI adoption" OR "generative AI") AND ("employment" OR "labor"); and title searches for all AEI-related works. OpenAlex title searches for "Anthropic Economic Index" returned 6 works; none concern tourism. Search strings and date-stamped outputs are stored in the replication repository (`review/novelty_search_log.md`).

## Appendix B. Robustness tables

Tables B1–B3 (alternative intensity definitions; top-5 exclusion; April-vs-May stability) are generated by `code/02_analysis.py` and stored in `results/` as `robustness_*.csv`.
