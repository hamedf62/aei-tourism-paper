...
Tables B1–B4 (alternative intensity definitions; top-5 exclusion; April-vs-May stability; winsorization, component-wise and population-weighted regressions) are generated by `code/07_robustness.py` and `code/13_supplementary_robustness.py` and stored in `results/` as `robustness_*.csv`.
